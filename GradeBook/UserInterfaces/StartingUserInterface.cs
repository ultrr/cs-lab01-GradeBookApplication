...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command) //was void
        {
            var parts = command.Split(' ');
            if (parts.Length != 4) //was 2 //3
            {
                Console.WriteLine("Command not valid, Create requires a name, type of gradebook, if it's weighted (true / false).");
                return;
            }
            bool weightcheck = bool.Parse(parts[3]);
            ...
            if (type == "standard")
            {
                gradeBook = new StandardGradeBook(name, weightcheck);
                Console.WriteLine("Created gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else if (type == "ranked")
            {
                gradeBook = new RankedGradeBook(name, weightcheck);
                Console.WriteLine("Created gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
